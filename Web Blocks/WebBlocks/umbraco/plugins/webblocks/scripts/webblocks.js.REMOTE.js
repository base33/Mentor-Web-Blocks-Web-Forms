/**
 * Builder class for Web Blocks. The main class that powers the builder interface.
 * Contains methods for fetching a layouts' blocks and containers definitions 
 * and the logic for rendering them.
 *
 * NOTE: Depends on jQuery to work properly.
 *
 * Copyright (c) 2012 Mentor Digital
 */
...
$(document).ready(function () {
    ...
    function addBlockToContainer(id, containerElement) {
        ...
        $.get(wbServiceUrl + '/RenderBlock/' + id + '?id=' + wbCurrentNodeId, function (data) {
            ...
            var blockElement = $('<div></div>');
            blockElement.addClass('block');
            blockElement.jeegoocontext('wbBlockContextMenu', contextMenuOptions);
            blockElement.append(data);
    ...
    function editBlock(blockElement) {
        ...
        // If the block model node id is 0 assume it's a wysiwyg.
        // In future let's actually make use of block type.
        // For now this is ok as I can't think of any use case
        // where the node id would be zero other than wysiwyg
        if (blockModel.getNodeId() == 0) {
            ...
            // Sort this s*** out!
            $('#editBlockIframe').css({ width: 593, height: 500 });
        }
    }
    ...
    // TODO: Sort this conditional statement looks ugly!
        ...
        $('.container').each(function () {
            ...
            $('.block', containerElement).each(function (index, value) {
                ...
                if (typeof (existingBlock) === 'undefined') {
                    ...
                    // Predefined will always be true as we know it's came
                    // from the template.
    ...
    // Set an interval to keep the containers serialisation up to date
    var serializeIntervalId = setInterval(function () {
    ...
    if ($.browser.msie) {
        ...
        $('form').submit(function (e) {
            //if (!formSubmitted) {
            //e.preventDefault();
            clearInterval(serializeIntervalId);
            $('.block').each(function () {
                var $this = $(this);
                $this.remove();
            });
    ...
    var $tinymceDialog = $('#tinymce').dialog({
        ...
        open: function () {
            ...
            initTinyMCE(function () {
                ...
                tinyMCE.activeEditor.toolbarElement.style.display = 'block';
            });
        },
